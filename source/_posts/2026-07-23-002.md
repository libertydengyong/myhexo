---
title: DMIT VPS IPv6 连接失败与服务访问排查记录
...
<script type="application/ld+json">
{
  ...
  "@graph": [
    {
      "@type": "BlogPosting",
      "headline": "DMIT VPS IPv6 连接失败与服务访问排查记录",
      "description": "记录DMIT VPS IPv6连接失败的实际排查过程，包括IPv6地址、默认路由、SSH、防火墙和DNS AAAA记录检查方法。",
      "datePublished": "2026-07-23T16:30:00+08:00",
      "dateModified": "2026-07-23T16:30:00+08:00",
      "url": "https://vpsjq.com/2026/07/23/2026-07-23-002/",
      "author": {
        "@type": "Organization",
        "name": "vpsjq.com"
      },
    ...
    },
    {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "DMIT VPS IPv6地址正常但SSH连不上怎么排查？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "先用ss -tlnp | grep ssh查看SSH是否监听IPv6，如果只看到0.0.0.0:22说明只监听IPv4，需要在/etc/ssh/sshd_config里确认AddressFamily any，然后重启SSH服务。"
          }
        },
        {
          "@type": "Question",
          "name": "有IPv6地址但网络不通怎么办？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "先用ip -6 addr确认地址是scope global（公网地址而不是fe80::本地链路地址），再用ip -6 route确认存在default路由，没有默认路由的话服务器虽然有IPv6地址但无法访问IPv6网络，需要检查VPS服务商提供的IPv6网关配置。"
          }
        },
        {
          "@type": "Question",
          "name": "网站有AAAA记录但还是访问不了怎么排查？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "用dig AAAA域名确认解析地址和VPS当前IPv6地址是否一致，同时检查Nginx是否监听IPv6（ss -tlnp | grep nginx应该看到:::80和:::443），以及IPv6防火墙ip6tables -L是否有DROP或REJECT规则拦截。"
          }
        }
      ]
    }
...
![DMIT VPS IPv6连接问题排查记录](/images/dmit-vps-ipv6.webp)

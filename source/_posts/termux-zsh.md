...
oh-my-zsh 安装完之后默认会启用一些插件，如果想进一步自定义，配置文件在 `~/.zshrc`，可以在里面启用更多插件或者换主题。常用的几个插件比如 `git`（显示当前 git 分支状态）、`z`（快速跳转到常用目录）都可以在配置文件里加上。不过插件不是装越多越好，装太多启动速度会变慢，挑几个真正用得上的就行。除了 `git` 和 `z`，`zsh-autosuggestions`（根据历史命令自动给出灰色补全建议，按右方向键采纳）和 `zsh-syntax-highlighting`（命令输入正确时变绿、错误时变红，一眼就能看出命令有没有敲对）这两个插件也很实用，装法是先把插件仓库克隆到 oh-my-zsh 的插件目录，再到 `~/.zshrc` 里的插件列表加上插件名。

如果觉得默认主题不够好看，`powerlevel10k` 是目前比较流行的一个主题，安装完之后会有一个交互式的配置向导，跑一遍就能按自己喜好定制提示符显示的内容（比如当前目录、git 状态、执行时间这些）。
